            ...
            if ($data4WoMo_params['table'] != 'N/A') {
                ...
                switch(strtolower($data4WoMo_params['table'])) {
                    ...
                    case "reisen_info":
                        ...
                        break;
                    case "reisen_umgebungsinfo":
                        $data4WoMo_sql = "SELECT name, CONCAT((select temperature from environment_data where sensor_id=9 and created_at > journey_begins and created_at < journey_ends order by temperature ASC LIMIT 1),' °C') as inside_tempmin, CONCAT((select temperature from environment_data where sensor_id=9 and created_at > journey_begins and created_at < journey_ends order by temperature DESC LIMIT 1),' °C') as inside_tempmax, CONCAT((select ROUND(AVG(temperature),1) from environment_data where sensor_id=9 and created_at > journey_begins and created_at < journey_ends),' °C') as inside_tempavg, CONCAT((select humidity from environment_data where sensor_id=9 and created_at > journey_begins and created_at < journey_ends order by humidity ASC LIMIT 1),' %') as inside_hummin, CONCAT((select humidity from environment_data where sensor_id=9 and created_at > journey_begins and created_at < journey_ends order by humidity DESC LIMIT 1),' %') as inside_hummax, CONCAT((select ROUND(AVG(humidity),0) from environment_data where sensor_id=9 and created_at > journey_begins and created_at < journey_ends),' %') as inside_humavg FROM journeys order by journey_begins ASC;";
